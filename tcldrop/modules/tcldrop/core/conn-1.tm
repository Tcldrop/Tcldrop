...
proc ::tcldrop::core::conn::Connect {opts} {
	...
	if {[catch { registeridx [set idx [assignidx]] [concat [array get options] [list idx $idx timestamp [clock seconds] sock [set sock [::proxy::connect $options(-proxychain) -command [list ::tcldrop::core::conn::ProxyControl $idx] {*}[array get options]]] remote "$options(-address):$options(-port)" info $info]] } error]} {
