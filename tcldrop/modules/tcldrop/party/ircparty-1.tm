...
#	Depends: partyline.
#
# $Id: ircparty.tcl,v 1.7 2005/07/31 04:17:26 fireegl Exp $
#
...
namespace eval ::tcldrop::ircparty {
	variable version {0.1}
	variable name {ircparty}
	variable depends {partyline irc server channels console core::users core::dcc core::conn core}
	variable author {Tcldrop-Dev}
	variable description {Provides the IRC interface for users.}
	variable rcsid {$Id: ircparty.tcl,v 1.7 2005/07/31 04:17:26 fireegl Exp $}
	variable script [info script]
	variable commands [list ircparty callircparty setircparty getircparty unsetircparty]
	# Provide the ircparty module:
	package provide tcldrop::$name $version
	# This makes sure we're loading from an eggdrop/tcldrop environment:
	if {![info exists ::numversion] || ![info exists ::tcldrop]} { return }
	# Export all the commands that should be available to 3rd-party scripters:
	eval [linsert $commands 0 namespace export]
}

proc ::tcldrop::ircparty::Connect {idx} {
	# Next line is used only during testing:
	uplevel #0 { source [file join modules party ircparty.tcl] }
	setidxinfo $idx [list -control {::tcldrop::ircparty::Read} -writable {::tcldrop::ircparty::Write} -errors {::tcldrop::ircparty::Error} state {CONNECT} other {conn} handle {*} module {ircparty} timestamp [clock seconds]]
	setircparty user $idx idx $idx nick {*} nickname "*@${::botnet-nick}:$idx" mode {w} username {-} realname {?}
	if {![info exists ::tcldrop]} { control $idx ::tcldrop::ircparty::Read }
	Write $idx
}

proc ::tcldrop::ircparty::Write {idx} {
...
proc ::tcldrop::ircparty::Read {idx line} {
	if {$line != {}} {
		array set idxinfo [getidxinfo $idx]
...
proc ::tcldrop::ircparty::Error {idx {error {}}} {
	#array set idxinfo [getidxinfo $idx]
	#callparty quit ${idx}:$idxinfo(handle)@${::botnet-nick}
	return 1
}

proc ::tcldrop::ircparty::Welcome {idx} {
...
proc ::tcldrop::ircparty::setircparty {type args} {
...
proc ::tcldrop::ircparty::getircparty {type args} {
	switch -- [string tolower $type] {
		{chanuser} - {chanusers} {
			variable IRCParty_chanusers
			variable IRCParty_users
...
proc ::tcldrop::ircparty::ircparty {command args} {
	switch -- $command {
		{userlist} {
			variable IRCParty_users
...
# putircparty -flags n
proc ::tcldrop::ircparty::putircparty {args} {
...
proc ::tcldrop::ircparty::PutRAW {idx msg} {
	putidx $idx ":${::botnet-nick} $msg"
}

proc ::tcldrop::ircparty::PutRAWClient {idx code msg} {
	PutRAW $idx "$code [idxinfo $idx nickname] $msg"
}

proc ::tcldrop::ircparty::ircparty_PING {idx command arg} {
	PutRAW $idx "PONG $arg"
	return 1
}

proc ::tcldrop::ircparty::ircparty_PONG {idx command arg} {
	return 1
}

proc ::tcldrop::ircparty::ircparty_NICK {idx command arg} {
	putlog "IN ::tcldrop::ircparty::NICK $idx $command $arg"
...
proc ::tcldrop::ircparty::ircparty_USER {idx command arg} {
	putlog "IN ::tcldrop::ircparty::USER $idx $command $arg"
...
proc ::tcldrop::ircparty::ircparty_OPER {idx command arg} {
...
proc ::tcldrop::ircparty::ircparty_LOGIN {idx command arg} { ircparty_OPER $idx $command $arg }
proc ::tcldrop::ircparty::ircparty_LOGON {idx command arg} { ircparty_OPER $idx $command $arg }
proc ::tcldrop::ircparty::ircparty_PASSWORD {idx command arg} { ircparty_PASS $idx $command $arg }
proc ::tcldrop::ircparty::ircparty_PASS {idx command arg} {
	# FixMe: Prevent people from using the PASS command too many times.
	foreach {handle password} [split $arg :] {break}
	ircparty_OPER $idx $command "$handle $password"
}

proc ::tcldrop::ircparty::ircparty_QUIT {idx command arg} {
	#unsetparty $idx
...
proc ::tcldrop::ircparty::ircparty_MODE {idx command arg} {
	putlog "in ::tcldrop::ircparty::ircparty_MODE $idx $command $arg"
...
proc ::tcldrop::ircparty::ircparty_USERHOST {idx command arg} {
	putlog "in ::tcldrop::ircparty::ircparty_USERHOST $idx $command $arg"
	array set userinfo [getircparty user $idx]
	switch -- $arg {
		$userinfo(nickname) {
...
proc ::tcldrop::ircparty::ircparty_VERSION {idx command arg} {
	putlog "in ::tcldrop::ircparty::ircparty_VERSION $idx $command $arg"
	return 1
}

proc ::tcldrop::ircparty::ircparty_ISON {idx command arg} {
	#putlog "in ::tcldrop::ircparty::ircparty_ISON $idx $command $arg"
	return 1
}

proc ::tcldrop::ircparty::ircparty_JOIN {idx command arg} {
...
proc ::tcldrop::ircparty::ircparty_NAMES {idx command arg} {
	array set userinfo [getircparty user $idx]
	array set idxinfo [getidxinfo $idx]
...
proc ::tcldrop::ircparty::ircparty_TOPIC {idx command arg} {
	array set userinfo [getircparty user $idx]
	if {[validchan [set chan [string trimleft $arg :]]]} {
...
proc ::tcldrop::ircparty::ircparty_PART {idx command arg} {
	return 1
}
...
proc ::tcldrop::ircparty::ircparty_WHO {idx command arg} {
	set chan $arg
...
proc ::tcldrop::ircparty::ircparty_WHOIS {idx command arg} {
	#set nick [getircparty user $idx nickname]
...
proc ::tcldrop::ircparty::RAWCapture {raws idx} {
	variable RAWCaptures
	foreach r $raws {
		bind raw - $r ::tcldrop::ircparty::RAWCapture_RAW
		set RAWCaptures($r,$idx) [list idx $idx last [lindex $raws end]]
	}
	lappend RAWCaptures($r,$idx) raws $raws afterid [after 9999 [list ::tcldrop::ircparty::RAWCapture_unset $raws $idx]]
}

proc ::tcldrop::ircparty::RAWCapture_unset {raws idx} {
	variable RAWCaptures
	foreach r $raws {
		array unset RAWCaptures $r,$idx
		if {![llength [array names RAWCaptures $r,*]]} { catch { unbind raw - $r ::tcldrop::ircparty::RAWCapture_RAW } }
	}
}

proc ::tcldrop::ircparty::RAWCapture_RAW {from key arg} {
	variable RAWCaptures
	foreach a [array names RAWCaptures $key,*] {
		...
		if {[string equal -nocase $rawinfo(last) $key]} {
			after idle [list ::tcldrop::ircparty::RAWCapture_unset $rawinfo(raws) $rawinfo(idx)]
...
proc ::tcldrop::ircparty::ircparty_PRIVMSG {idx command arg} {
...
bind sign - * ::tcldrop::ircparty::SIGN
proc ::tcldrop::ircparty::SIGN {nick host hand chan reason} {
...
bind splt - * ::tcldrop::ircparty::SPLT
proc ::tcldrop::ircparty::SPLT {nick host hand chan} {
	# FixMe: Just make this a RAW bind to QUIT.
}

bind rejn - * ::tcldrop::ircparty::REJN
proc ::tcldrop::ircparty::REJN {nick host hand chan} {
...
bind join - * ::tcldrop::ircparty::JOIN
proc ::tcldrop::ircparty::JOIN {nick host hand chan} {
	# Send to ircparty users who are on $chan:
	putircparty -chan $chan ":$nick!$host JOIN :$chan"
}

bind part - * ::tcldrop::ircparty::PART
proc ::tcldrop::ircparty::PART {nick host hand chan msg} {
	# Send to ircparty users who are on $chan:
	putircparty -chan $chan ":$nick!$host PART $chan"
}

bind topc - * ::tcldrop::ircparty::TOPC
proc ::tcldrop::ircparty::TOPC {nick host hand chan topic} {
	# Send to ircparty users who are on $chan:
	putircparty -chan $chan ":$nick!$host TOPIC $chan :$topic"
}

bind kick - * ::tcldrop::ircparty::KICK
proc ::tcldrop::ircparty::KICK {nick host hand chan target reason} {
	# Send to ircparty users who are on $chan:
	putircparty -chan $chan ":$nick!$host KICK $chan $target :$reason"
}

bind nick - * ::tcldrop::ircparty::NICK
proc ::tcldrop::ircparty::NICK {nick host hand chan newnick} {
	# Send to ircparty users who are on $chan:
	# FixMe: Make this a RAW bind, because NICK's are NOT channel specific.
	putircparty -chan $chan ":$nick!$host NICK :$newnick"
}

bind mode - * ::tcldrop::ircparty::MODE
proc ::tcldrop::ircparty::MODE {nick host hand chan mode {victim {}}} {
	# Send to ircparty users who are on $chan:
...
bind pubm - * ::tcldrop::ircparty::PUBM
proc ::tcldrop::ircparty::PUBM {nick host hand chan text} {
	# Send to ircparty users who are on $chan:
	putircparty -chan $chan ":$nick!$host PRIVMSG $chan :$text"
}

bind msgm - * ::tcldrop::ircparty::MSGM
proc ::tcldrop::ircparty::MSGM {nick host hand text} {
	# Send to all ircparty users with global +n
	putircparty -flags n ":$nick!$host PRIVMSG $::botnick :$text"
}

bind ctcp - * ::tcldrop::ircparty::CTCP
proc ::tcldrop::ircparty::CTCP {nick host hand dest keyword text} {
...
bind ctcr - * ::tcldrop::ircparty::CTCR
proc ::tcldrop::ircparty::CTCR {nick host hand dest keyword text} {
	if {[validchan $dest]} {
...
bind notc - * ::tcldrop::ircparty::NOTC
proc ::tcldrop::ircparty::NOTC {nick host hand text dest} {
	if {[validchan $dest]} {
...
bind wall - * ::tcldrop::ircparty::WALL
proc ::tcldrop::ircparty::WALL {hand msg} {
...
if {[info exists ::tcldrop]} {
	...
	proc ::tcldrop::ircparty::callircparty {idx command arg} {
	...
	bind load - ircparty ::tcldrop::ircparty
	proc ::tcldrop::ircparty {module} {
		...
		addlistentype ircparty connect ::tcldrop::ircparty::Connect ident 1 dns 1
		bind ircparty - PING ::tcldrop::ircparty::ircparty_PING
		bind ircparty - PONG ::tcldrop::ircparty::ircparty_PONG
		bind ircparty - NICK ::tcldrop::ircparty::ircparty_NICK
		bind ircparty - USER ::tcldrop::ircparty::ircparty_USER
		bind ircparty - OPER ::tcldrop::ircparty::ircparty_OPER
		bind ircparty - WHO ::tcldrop::ircparty::ircparty_WHO
		bind ircparty - LOGIN ::tcldrop::ircparty::ircparty_LOGIN
		bind ircparty - LOGON ::tcldrop::ircparty::ircparty_LOGON
		bind ircparty - PASSWORD ::tcldrop::ircparty::ircparty_PASSWORD
		bind ircparty - PASS ::tcldrop::ircparty::ircparty_PASS
		bind ircparty - QUIT ::tcldrop::ircparty::ircparty_QUIT
		bind ircparty - MODE ::tcldrop::ircparty::ircparty_MODE
		bind ircparty - USERHOST ::tcldrop::ircparty::ircparty_USERHOST
		bind ircparty - VERSION ::tcldrop::ircparty::ircparty_VERSION
		bind ircparty - ISON ::tcldrop::ircparty::ircparty_ISON
		bind ircparty - JOIN ::tcldrop::ircparty::ircparty_JOIN
		bind ircparty - PART ::tcldrop::ircparty::ircparty_PART
		bind ircparty - NAMES ::tcldrop::ircparty::ircparty_NAMES
		bind ircparty - TOPIC ::tcldrop::ircparty::ircparty_TOPIC
		bind ircparty - WHOIS ::tcldrop::ircparty::ircparty_WHOIS
		bind ircparty - PRIVMSG ::tcldrop::ircparty::ircparty_PRIVMSG
		# Don't allow the ircparty module to be unloaded:
		bind unld - ircparty ::tcldrop::ircparty::UNLD
		proc ::tcldrop::ircparty::UNLD {module} { return 1 }
	}
} elseif {[info exists ::numversion]} {
	# Special stuff for Eggdrop..
	proc ::tcldrop::ircparty::callircparty {idx command arg} {
		if {[llength [info commands ::tcldrop::ircparty::ircparty_$command]]} {
			if {[catch { ::tcldrop::ircparty::ircparty_$command $idx $command $arg } err]} {
				putlog "Error in ::tcldrop::ircparty::ircparty_${command}: $err"
	...
	proc ::tcldrop::ircparty::killidx {idx args} {
	...
	proc ::tcldrop::ircparty::sock2idx {sock} { lindex [listidx [list sock $sock]] 0 }

	proc ::tcldrop::ircparty::killsock {sock} { killidx [lindex [listidx [list sock $sock]] 0] }

	proc ::tcldrop::ircparty::idx2sock {idx} { idxinfo $idx sock }

	# Assigns a unique idx handle for a connection:
	proc ::tcldrop::ircparty::assignidx {args} {
		variable IDXCount
		incr IDXCount
	}
	...
	proc ::tcldrop::ircparty::registeridx {idx {info {}}} { set ::idxlist($idx) $info }

	# Unregisters an idx:
	proc ::tcldrop::ircparty::unregisteridx {idx} { array unset ::idxlist $idx }
	...
	proc ::tcldrop::ircparty::initidx {{info {}}} {
		variable IDXCount
	...
	proc ::tcldrop::ircparty::idxlist {{info {}}} {
		global idxlist
		switch -- $info {
			{} { array get idxlist }
			{default} {
	...
	proc ::tcldrop::ircparty::listidx {{info {}}} {
		global idxlist
		switch -- $info {
			{} { array get idxlist }
			{default} {
	...
	proc ::tcldrop::ircparty::dccused {} { array size ::idxlist }
	...
	proc ::tcldrop::ircparty::idxinfo {idx {info {}} {value {}}} {
		if {[info exists ::idxlist($idx)]} {
			switch -- $info {
				{} { return $::idxlist($idx) }
				{default} {
	...
	proc ::tcldrop::ircparty::setidxinfo {idx {info {}}} {
	...
	proc ::tcldrop::ircparty::getidxinfo {idx} { if {[info exists ::idxlist($idx)]} { set ::idxlist($idx) } }

	# Deletes a piece of info about the idx (to save a tad of memory):
	proc ::tcldrop::ircparty::delidxinfo {idx info} {
		if {[info exists ::idxlist($idx)]} {
			array set idxinfo $::idxlist($idx)
	...
	proc ::tcldrop::ircparty::valididx {idx} { info exists ::idxlist($idx) }

	# Returns the idx that $handle is on:
	proc ::tcldrop::ircparty::hand2idx {handle} {
		# FixMe: Make it return the last active user.
		lindex [listidx [list handle $handle]] 0
	}

	proc ::tcldrop::ircparty::idx2hand {idx} { idxinfo $idx handle }
}
